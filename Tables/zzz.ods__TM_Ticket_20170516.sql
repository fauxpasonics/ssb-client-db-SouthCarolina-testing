CREATE TABLE [zzz].[ods__TM_Ticket_20170516]
(
...
[InsertDate] [datetime] NULL CONSTRAINT [DF__TM_Ticket__Inser__489AC854] DEFAULT (getdate()),
[UpdateDate] [datetime] NULL CONSTRAINT [DF__TM_Ticket__Updat__498EEC8D] DEFAULT (getdate()),
...
GO
ALTER TABLE [zzz].[ods__TM_Ticket_20170516] ADD CONSTRAINT [PK_TM_Ticket] PRIMARY KEY CLUSTERED  ([id])
GO
CREATE NONCLUSTERED INDEX [IDX_BusinessKey] ON [zzz].[ods__TM_Ticket_20170516] ([acct_id], [plan_event_id], [ticket_status], [section_id], [row_id], [seat_num]) INCLUDE ([num_seats], [ssbPriceCode])
GO
CREATE NONCLUSTERED INDEX [IDX_TM_Tkt_ticket_status] ON [zzz].[ods__TM_Ticket_20170516] ([num_seats], [ticket_status], [acct_id], [plan_event_id], [seat_num], [section_id], [row_id]) INCLUDE ([id])
GO
CREATE NONCLUSTERED INDEX [IDX_ticket_status] ON [zzz].[ods__TM_Ticket_20170516] ([ticket_status])
GO
CREATE NONCLUSTERED INDEX [IDX_ticket_status_add_datetime] ON [zzz].[ods__TM_Ticket_20170516] ([ticket_status], [add_datetime]) INCLUDE ([block_purchase_price], [event_id], [event_name], [num_seats], [plan_event_id], [price_code], [Price_code_desc], [row_id], [seat_num], [section_id])
GO
CREATE NONCLUSTERED INDEX [Idx_upd_datetime_event_id] ON [zzz].[ods__TM_Ticket_20170516] ([upd_datetime], [acct_id], [event_id])
GO
CREATE NONCLUSTERED INDEX [IDX_UpdateDate] ON [zzz].[ods__TM_Ticket_20170516] ([UpdateDate])
GO
