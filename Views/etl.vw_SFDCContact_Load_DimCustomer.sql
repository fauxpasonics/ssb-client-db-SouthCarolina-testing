...
CREATE VIEW [etl].[vw_SFDCContact_Load_DimCustomer] AS (
...
*/
		...
		SELECT
			'SouthCarolina SFDC' SourceDB
			,'SouthCarolina PC_SFDC Contact' SourceSystem
			...
			, CAST(p.SSB_CRMSYSTEM_TM_Account_ID__c as varchar(100)) AS [ExtAttribute5] 
			, NULL [ExtAttribute6] 
			...
			, CASE WHEN (SELECT value FROM [dbo].[fn_Split](p.SSB_CRMSYSTEM_TM_Account_ID__c,', ') where position = 1) is not null
				THEN 'TM-' + (SELECT value FROM [dbo].[fn_Split](p.SSB_CRMSYSTEM_TM_Account_ID__c,', ') where position = 1)
				END customer_matchkey

--		select top 100 *
		FROM  SouthCarolina_Reporting.ProdCopy.Contact p	--order by SSB_CRMSYSTEM_Contact_ID__c desc	
		WHERE p.isdeleted = 0
