...
CREATE PROCEDURE [mdm].[UpdateSSBPrimaryFlag_Acct]
(
	@ClientDB VARCHAR(50)
)
AS
...
SET @sql = @sql
	...
	+ 'set  ssb_crmsystem_acct_primary_flag = case when ranking = 1 then 1 else 0 end ' + CHAR(13)
	+ 'from ' + @clientDb + 'dbo.dimcustomerssbid ssbid' + CHAR(13)
	+ 'inner join (' + CHAR(13)
	+ 'select *,' + CHAR(13)
	+ 'rank() over (partition by isnull(ssb_crmsystem_acct_id, ssb_crmsystem_contact_id) order by ' + @RankingList + ' ssb_crmsystem_primary_flag desc, ssb_crmsystem_acct_primary_flag desc,  DimCustomerId) as ranking' + CHAR(13)
...
SET @sql = @sql + CHAR(13) + CHAR(13)
	...
	SET @sql = @sql
	+ 'Insert into ' + @clientDb + 'mdm.auditlog (logdate, mdm_process, process_step, cnt)' + CHAR(13)
	+ 'values (current_timestamp, ''Primary Flag'', ''set acct primary'', @@ROWCOUNT);' 
SET @sql = @sql + CHAR(13) + CHAR(13)
