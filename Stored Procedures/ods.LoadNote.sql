...
WHEN MATCHED AND (source.SrcHashKey <> target.HashKey or isnull(source.text,'') <> isnull(target.text,'') or isnull(source.task_stage_text,'') <> isnull(target.task_stage_text,''))
